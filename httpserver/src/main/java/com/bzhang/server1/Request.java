...
public class Request {
	...
	/**
	 * 初始化request，并读取requestInfo信息
	 * @param socket
	 * @throws IOException
	 */
	public Request(SocketChannel socket) {
		this();
		int read =0;
		...
		try {
			do {
				...
				if (read>0) {
					...
					requestInfo.append(new String(readBuffer.array(), "utf-8"));
				}
			} while (read>0);
	...
	private void parseRequestInfo() {
		...
		if (method.equalsIgnoreCase("get")) {
			if (urlStr.contains("?")) {
				url=urlStr.substring(requestHead.indexOf("/"),requestHead.indexOf("?")-1);
				param=urlStr.substring(requestHead.indexOf("?")+1).trim();
	...
	public void buildParamMap(String param) {
		...
		for (String para : params) {
			...
			if (!parameter.containsKey(sp[0])) {
				...
				list.add(decode(sp[1], "utf-8"));
				parameter.put(sp[0], list);
			}else {
				parameter.get(sp[0]).add(decode(sp[1], "utf-8"));
			}
